...
public function get_comment($user_id,$status=2,$p){
     ...
     * @return array
     */
    public function add_ruzhu($user_id,$data){
        $post = $data;

        //检查手机格式
        if($post['user_name'] == '')
            return array('status'=>-1,'msg'=>'联系人不能为空','result'=>'');

        if(!$post['lbtype'])
            return array('status'=>-1,'msg'=>'经营类别不能为空','result'=>'');
        if(!check_mobile($post['mobile']))
            return array('status'=>-1,'msg'=>'手机号码格式有误','result'=>'');
        //判断提交的手机号码是否为重复，重复不能提交
        $a=M('user_ruzhu')->where(array('mobile'=>$post['mobile']))->count();
        if($a){return array('status'=>-1,'msg'=>'您已经提交申请过了，不能重复提交申请！','result'=>'');}
        ...
        $post['reg_time']=time();
        $inser_id = M('user_ruzhu')->add($post);
        if(!$inser_id)
            return array('status'=>-1,'msg'=>'添加失败','result'=>'');


        return array('status'=>1,'msg'=>'添加成功','result'=>$inser_id);
    }
}